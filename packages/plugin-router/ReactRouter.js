...
const templates = [
  ...
  result[template] = file.readFile(
    path.resolve(__dirname, `./templates-react/${template}`),
  );
  return result;
}, {});
...
class ReactRouter extends Router {
  ...
  writeSuspense() {
    ...
    const suspenseFilePath = path.resolve(
      this.options.outputPath,
      suspenseFileName,
    );
    const { loading = {} } = this.options.appConfig;
    const loadingData = { loading: {} };
    loadingData.loading.page = loading.page
      ? this.relativePath(suspenseFilePath, loading.page)
      : "";
    loadingData.loading.layout = loading.layout
      ? this.relativePath(suspenseFilePath, loading.layout)
      : "";

    this.writeTemplate(
      suspenseFileName,
      templates["Suspense.ejs"],
      loadingData,
    );
  }
  ...
  writeLayout(layout) {
    const relativeRawLayoutFilePath = this.relativePath(
      layout.dynamicLayoutFilePath,
      layout.rawLayoutFilePath,
    );
    this.writeTemplate(
      layout.dynamicLayoutFilePath,
      templates["dynamicLayout.ejs"],
      {
        ...layout,
  ...
  writePage(page) {
    const relativeRawPageFilePath = this.relativePath(
      page.dynamicPageFilePath,
      page.rawPageFilePath,
    );
