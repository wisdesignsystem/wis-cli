...
export default function (plugin, options = []) {
  function getExpose({ context, filePath, option }) {
    ...
    const item = parentDir
      .replace(context.path.src, "")
      .split(path.sep)
      .filter(Boolean);
    item.push(
      is.isFunction(option.getModuleName) ? option.getModuleName(name) : name,
    );
    item.push(chunkName);
  ...
  plugin.hooks.context.tap((context) => {
    plugin.hooks.eachFile.tap((filePath) => {
      ...
      const isExist = matchedChunkOptions.some(
    ...
    plugin.hooks.webpackConfigure.tap((webpackConfigure) => {
            ...
            babel.traverse.default(ast, {
              VariableDeclarator: (path) => {
                if (path.node.id.name === "get") {
                  const node = babel.parser.parse(
                    generateScopeMatchCode(matchedChunkOptions),
                  );
      ...
      const exposePaths = Object.keys(crossExpose).reduce((result, name) => {
        const exposePath = path.resolve(
          tool.replaceAlias(context.config.alias, crossExpose[name]),
        );
