...
export default function (plugin, options) {
  ...
  function mobileCss(webpackConfigure) {
    ...
    const postcssPlugin = mobileCss.get(
      "use.postcss.options.postcssOptions.plugins",
    );
  ...
  function mobileCssModule(webpackConfigure) {
    ...
    const mobileCssModule = webpackConfigure.get(
      "module.rules.mobileCssModule",
    );
    mobileCssModule.set("test.mobileCssModule", /mobile\/.*\.module\.css$/);

    const postcssPlugin = mobileCssModule.get(
      "use.postcss.options.postcssOptions.plugins",
    );
    postcssPlugin.set("pxToViewport", []);
    postcssPlugin.set(
  ...
  function mobileLess(webpackConfigure) {
    ...
    const postcssPlugin = mobileLess.get(
      "use.postcss.options.postcssOptions.plugins",
    );
    postcssPlugin.set("pxToViewport", []);
    postcssPlugin.set(
  ...
  function mobileLessModule(webpackConfigure) {
    ...
    const mobileLessModule = webpackConfigure.get(
      "module.rules.mobileLessModule",
    );
    mobileLessModule.set("test.mobileLessModule", /mobile\/.*\.module\.less$/);

    const postcssPlugin = mobileLessModule.get(
      "use.postcss.options.postcssOptions.plugins",
    );
    postcssPlugin.set("pxToViewport", []);
    postcssPlugin.set(
    ...
    if (pluginData.path.includes("@wisdesign/plugin-import-demand")) {
      pluginData.option.push({
        ...
        scope: (module, moduleMap) => {
          ...
          if (
            /\b(BlackBerry|webOS|iPhone|IEMobile|Android|Windows Phone|iPod)\b/i.test(
              UA,
            )
          ) {
            agent = "mobile";
