...
class RemoteWebpackPlugin {
  constructor(options = {}) {
    ...
    this.remoteScopeName = options.scopeName
      ? `${options.scopeName}:`
      : "remote:";
  ...
  apply(compiler) {
    if (
      !compiler.options.output.enabledLibraryTypes.includes(this.library.type)
    ) {
          ...
          normalModuleFactory.hooks.factorize.tap(
            ...
            (data) => {
              ...
              return new RemoteModule(
                remote.request,
                remote.externalRequests,
                remote.internalRequest,
                remote.shareScope,
              );
            },
          );
  ...
  getRemote(request) {
    let [remoteName, ...rest] = request
      .slice(this.remoteScopeName.length)
      .split("/");
