...
class Wis {
  ...
  async initial() {
    ...
    plugin.register(
      "webpackDevServerConfigure",
      new Tap(["webpackDevServerConfigure"]),
    );

    // webpack dev server配置设置勾子方法
    plugin.register(
      "webpackDevServerConfig",
      new TapStream(["webpackDevServerConfig"]),
    );
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      const watcher = chokidar.watch(
        [this.context.path.src].concat(configFiles),
        {
          ignored: [
            this.context.path.prodCompiler,
            this.context.path.devCompilerPath,
          ],
          ignoreInitial: true,
        },
      );
      watcher
        .on("add", ensureRestart)
        .on("change", ensureRestart)
        .on("unlink", ensureRestart);
